                    ...
                    <form onSubmit={handleSubmit((data) => {alert(JSON.stringify(data))})}
                        ...
                        <div className='flex flex-row gap-10 pt-4 pb-6'>
                            <div className='flex flex-col gap-2.5'>
                                ...
                                <Textarea styles={`${errors.shortDescription && 'border-red-300'}`} validationProps={register('shortDescription', {required: true})}/>
                            </div>
                            <div className='flex flex-col gap-2.5'>
                                <div className='flex flex-row items-end'>
                                    <p className='text-lg font-light text-[#208843]'>Ввідна лекція</p>
                                    <Tippy
                                        content='Метою ввідної лекції є ознайомлення студентів з вибірковою дисципліною та мотивація до її подальшого вивчення'>
                                        <div className='h-4 mb-[6px] ml-1.5 hover:cursor-pointer'>
                                            <AiOutlineInfoCircle size={17}/>
                                        </div>
                                    </Tippy>
                                ...
                                <div className='flex w-60 h-10'>
                                    <DateTimePicker validationProps={register('dateTime', {required: true})}
                                                    placeholder='Дата' styles={`${errors.dateTime && 'border-red-300'}`}/>
                                </div>
                                <div className='flex w-60 h-10'>
                                    <Input isURL={true} validationProps={register('link', {required: true})}
                                           placeholder='Посилання на лекцію' styles={`${errors.link && 'border-red-300'}`}/>
                                </div>
                            </div>
                        </div>
                        <div className='flex flex-row'>
                            <p className='whitespace-nowrap overflow-hidden text-ellipsis pt-[8px] w-96 text-xl font-normal'>{subject?.name}</p>
